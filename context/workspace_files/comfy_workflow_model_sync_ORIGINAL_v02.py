...
USER_AGENT = "Mozilla/5.0 (compatible; ComfyWorkflowModelSync/1.1)"
REPORT_NAME = "workflow_model_report.json"
MODEL_FILE_EXTENSIONS = {".safetensors", ".ckpt", ".pt", ".bin"}
...
def infer_dir_from_node(node_type: str, model_name: str) -> str:
    ...
    if "vae" in lower:
        return "vae"
    if "clip" in lower or "umt5" in lower or "text" in lower or "t5" in lower:
...
def extract_model_from_widgets(node: dict) -> List[ModelRef]:
    ...
    lower = first.lower()
    if not any(lower.endswith(ext) for ext in MODEL_FILE_EXTENSIONS):
        return []
...
def summarize_verification(results: List[ModelResult]):
    counts: Dict[str, int] = {}
    for r in results:
        counts[r.status] = counts.get(r.status, 0) + 1
...
def main():
    ...
    model_refs = get_model_refs_for_mode(workflow, mode if mode != "dry-run" else "active")
    ...
    summarize_results(sync_results)
    summarize_verification(verification_results)
